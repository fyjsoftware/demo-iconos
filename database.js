...
export async function mostrar(tabla, query, st) {
  ...
  if (query.dev !== null && query.cliente === null) {
    filtro = ' WHERE desarrollador LIKE "%' + query.dev + '%"';
  }
  if (query.cliente !== null && query.dev === null) {
    filtro = ' WHERE cliente LIKE "%' + query.cliente + '%"';
  }
  if (query.dev !== null && query.cliente !== null) {
    filtro =
      ' WHERE desarrollador LIKE "%' +
      query.dev +
      '%" AND cliente LIKE "%' +
      query.cliente +
      '%"';
  ...
  if (st == false) {
    ...
    let [resultados] = await database.execute(
      "SELECT desarrollador, cliente, proyecto, SUM(duracion) AS duracion from " +
        tabla +
        filtro +
        " GROUP BY desarrollador, cliente, proyecto;"
    );
    newR = resultados;
  }
  await database.end();
...
export async function mostrarListas(tabla) {
  let database = await conexion();
  const [devs] = await database.execute(
    "SELECT DISTINCT desarrollador FROM " + tabla + ";"
  );
  const [clientes] = await database.execute(
    "SELECT DISTINCT cliente FROM " + tabla + ";"
  );
  await database.end();
  let arr1 = [];
  let arr2 = [];
