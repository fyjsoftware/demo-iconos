		...
		try {
			try {
				...
				if ((error.code = "ENOENT")) {
                    let dev = request.query.desarrollador !== undefined ? request.query.desarrollador : null;
                    let cliente = request.query.cliente !== undefined ? request.query.cliente : null;
					await response.render("data", {
                        tabla: request.params.tabla,
                        listas: await db.mostrarListas(request.params.tabla),
						resultado: await db.mostrar(request.params.tabla, { dev: dev, cliente: cliente })
					});
				} else {
